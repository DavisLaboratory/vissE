...
An alternative application of this package is to characterise a gene list based on similarity with all other gene-sets in the MSigDB. Below, we demonstrate how a list of estrogen response genes can be characterised using this approach.

```{r}
#retrieve hallmarks gene-sets from this package
data(hgsc)
#create a query using Estrogen response genes
...
Next, we follow a similar workflow to the previous section whereby gene-set clusters are first, identified, then characterised.

```{r fig.width=8, fig.height=5}
#identify clusters
grps = cluster_walktrap(sim_ovnet)
#extract clustering results
grps = groups(grps)
#sort by cluster size
grps = grps[order(sapply(grps, length), decreasing = TRUE)]
...
#compute and plot the results of text-mining
p1 = plotMsigWordcloud(msigdb_hs, grps, type = 'Name')
set.seed(36) #set seed for reproducible layout
p2 = plotMsigNetwork(sim_ovnet, markGroups = grps)
