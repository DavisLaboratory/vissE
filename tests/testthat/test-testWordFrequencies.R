...
test_that("word frequencies (TFIDF) are computed correctly", {
  ...
  expect_equal(sapply(freq, nrow), c('Name' = 4, 'Short' = 5))
  expect_equal(sapply(freq, class), c('Name' = 'data.frame', 'Short' = 'data.frame'))
  expect_equal(freq$Name[freq$Name$word %in% c('late'), 2], 1.265124, tolerance = 1e-5)
  expect_equal(freq$Short[freq$Short$word %in% c('early', 'late'), 2], c(1.222462, 1.182552), tolerance = 1e-5)
  expect_equal(freq$Name[freq$Name$word %in% c('estrogen'), 2], 2.083008, tolerance = 1e-5)
  expect_equal(freq$Short[freq$Short$word %in% c('estrogen'), 2], 2.063877, tolerance = 1e-5)
  expect_false('HALLMARK' %in% freq$Name$word)
  expect_false('to' %in% freq$Short$word)
})
